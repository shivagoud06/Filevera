...
export default function AuthForm({ mode, googleConfigured = false }: { mode: "login" | "signup"; googleConfigured?: boolean }) {
    ...
    return (
        <form onSubmit={submit} className="mt-4 space-y-3.5" noValidate>
            {isSignup && (
                <div>
                    <label htmlFor="name" className="block text-xs font-semibold uppercase tracking-wider text-slate-700">Name</label>
                    <input
                        id="name"
                        required
                        value={name}
                        onChange={(event) => setName(event.target.value)}
                        className="mt-1 h-10 sm:h-11 w-full rounded-xl border border-slate-300 bg-white px-3.5 text-sm text-slate-900 outline-none transition-colors focus:border-sky-500 focus:ring-2 focus:ring-sky-200"
                    />
                </div>
            )}
            <div>
                <label htmlFor="email" className="block text-xs font-semibold uppercase tracking-wider text-slate-700">Email</label>
                <input
                    id="email"
                    type="email"
                    autoComplete="email"
            ...
            <div>
                ...
                <div className="mt-1 flex h-10 sm:h-11 overflow-hidden rounded-xl border border-slate-300 bg-white transition-colors focus-within:border-sky-500 focus-within:ring-2 focus-within:ring-sky-200">
                    <input
                        id="password"
                        ...
                        className="min-w-0 flex-1 px-3.5 text-sm text-slate-900 outline-none"
                    />
                    <button
                        type="button"
            ...
            {isSignup && (
                <div>
                    <label htmlFor="confirm-password" className="block text-xs font-semibold uppercase tracking-wider text-slate-700">Confirm password</label>
                    <input
                        id="confirm-password"
                        type={showPassword ? "text" : "password"}
            ...
            {isSignup && (
                <label className="flex items-start gap-2.5 pt-0.5 text-xs leading-4 text-slate-600">
                    <input
                        type="checkbox"
                        checked={agreed}
                    ...
                    />
                    <span>
                        I agree to the <Link href="/terms" className="font-semibold text-sky-600 hover:underline">Terms</Link> and <Link href="/privacy" className="font-semibold text-sky-600 hover:underline">Privacy Policy</Link>.
                    </span>
                </label>
            )}
            ...
            {!isSignup && (
                <Link href="/forgot-password" className="block text-center text-xs font-medium text-sky-600 hover:underline">
                    Forgot password?
                </Link>
            )}
            <div className="flex items-center gap-3 py-0.5 text-[11px] font-semibold uppercase tracking-wider text-slate-400">
